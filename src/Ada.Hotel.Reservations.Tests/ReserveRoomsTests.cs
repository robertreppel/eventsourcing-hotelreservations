...
namespace Ada.Hotel.Reservations.Tests
{
    ...
    public class ReserveRoomsTests
    {
        ...
        public void Must_reserve_at_least_one_room()
        {
            ...
            //When
            object[] events = null;
            TestDelegate when = () => events = rooms.Reserve("guest1", new DateTime(2016,03, 1), new DateTime(2016, 03, 2), 0).ToArray();
            //Then
        ...
        public void Checkout_cant_be_before_checkin()
        {
            ...
            //When
            object[] events = null;
            TestDelegate when = () => events = rooms.Reserve("guest1", new DateTime(2016, 03, 3), new DateTime(2016, 03, 2), 1).ToArray();
            //Then
            var ex = Assert.Throws<InvalidOperationException>(when);
            Assert.That(events, Is.Null);
        ...
        public void Checking_out_on_the_same_day_as_checking_in_is_not_OK()
        {
            ...
            //When
            object[] events = null;
            TestDelegate when = () => events = rooms.Reserve("guest1", new DateTime(2016, 03, 2), new DateTime(2016, 03, 2), 1).ToArray();
            //Then
            var ex = Assert.Throws<InvalidOperationException>(when);
            Assert.That(events, Is.Null);
            Assert.That(ex.Message, Is.EqualTo("Checkin date must be before checkout date."));
        ...
        public void Reserving_without_guestid_fails()
        {
            ...
            //When
            object[] events = null;
            TestDelegate when = () => events = rooms.Reserve("", new DateTime(2016, 04, 2), new DateTime(2016, 04, 7), 1).ToArray();
            //Then
            var ex = Assert.Throws<InvalidOperationException>(when);
            Assert.That(events, Is.Null);
        ...
        public void Reserving_a_room_type_for_which_there_are_no_other_reservations_is_OK()
        {
            ...
            var events = rooms.Reserve("guest 1", checkInDate, checkOutDate, 1).ToArray();
            //Then
        ...
        public void Reserving_more_rooms_than_exist_for_a_type_of_room_is_not_OK()
        {
            ...
            var checkOutDate = new DateTime(2016, 04, 7);
            object[] events = null;
            TestDelegate when = () => events = rooms.Reserve("guest 1", checkInDate, checkOutDate, 3).ToArray();
            //Then
            var ex = Assert.Throws<InvalidOperationException>(when);
            Assert.That(events, Is.Null);
        ...
        public void Making_a_reservation_which_starts_after_checkout_of_a_previous_reservation_is_OK()
        {
            ...
            rooms.Hydrate(new RoomsReserved(previousReservationCheckinDate, previousReservationCheckoutDate, roomTypeId, "guest 1", 1));
            //When
            var checkInDate = previousReservationCheckoutDate;
            var checkOutDate = new DateTime(2016, 5, 3);
            var events = rooms.Reserve("guest 2", checkInDate, checkOutDate, 1).ToArray();
            //Then
            Assert.AreEqual(1, events.Length);
            var roomsReserved = events[0] as RoomsReserved;
            Assert.That(roomsReserved, Is.TypeOf<RoomsReserved>());
        ...
        [Test]
        public void Making_a_reservation_which_ends_before_a_subsequent_reservation_is_OK()
        {
            //Given
            var rooms = new Rooms();
            var roomTypeId = Guid.NewGuid();
            ...
            rooms.Hydrate(new RoomsReserved(laterReservationCheckinDate, laterReservationCheckoutDate, roomTypeId, "guest 1", 1));
            //When
        ...
        public void New_reservation_with_same_checkin_and_checkout_dates_as_existing_one_is_not_OK()
        {
            ...
            rooms.Hydrate(new RoomsReserved(new DateTime(2016, 04, 02), new DateTime(2016, 04, 03), roomTypeId, "guest 1", noOfUnits));
            //When
            var checkInDate = new DateTime(2016, 04, 2);
            var checkOutDate = new DateTime(2016, 04, 3);
            object[] events = null;
            TestDelegate when = () => events = rooms.Reserve("guest 2", checkInDate, checkOutDate, noOfUnits).ToArray();
            //Then
            var ex = Assert.Throws<InvalidOperationException>(when);
            Assert.That(events, Is.Null);
        ...
        public void New_reservation_with_checkin_and_checkout_dates_within_existing_reservation_is_not_OK()
        {
            ...
            var noOfUnits = 1;
            rooms.Hydrate(new RoomsCreated(roomTypeId, "Double Room", noOfUnits, hotelId));
            rooms.Hydrate(new RoomsReserved(new DateTime(2016, 03, 30), new DateTime(2016, 04, 03), roomTypeId, "guest 1", noOfUnits));
            //When
            var checkInDate = new DateTime(2016, 04, 2);
        ...
        public void New_reservation_with_checkout_during_existing_reservation_is_not_OK()
        {
            ...
            var noOfUnits = 1;
            rooms.Hydrate(new RoomsCreated(roomTypeId, "Double Room", noOfUnits, hotelId));
            rooms.Hydrate(new RoomsReserved(new DateTime(2016, 04, 20), new DateTime(2016, 04, 22), roomTypeId, "guest 1", noOfUnits));
            //When
        ...
        public void Checking_in_same_day_as_previous_reservation_checkout_is_OK()
        {
            ...
            var events = rooms.Reserve("guest 2", checkInDate, checkOutDate, noOfUnits).ToArray();
        ...
        public void Multiple_conflicting_reservations_are_handled_correctly()
        {
            ...
            rooms.Hydrate(new RoomsCreated(roomTypeId, "Double Room", noOfUnits, hotelId));
            rooms.Hydrate(new RoomsReserved(new DateTime(2016, 04, 20), new DateTime(2016, 04, 22), roomTypeId, "guest 1", noOfUnits));
            rooms.Hydrate(new RoomsReserved(new DateTime(2016, 04, 20), new DateTime(2016, 04, 22), roomTypeId, "guest 3", noOfUnits));
            //When
            var checkInDate = new DateTime(2016, 04, 19);
            var checkOutDate = new DateTime(2016, 04, 23);
            object[] events = null;
        ...
        public void New_reservations_when_all_existing_rooms_are_full_is_not_OK()
        {
            ...
            rooms.Hydrate(new RoomsReserved(new DateTime(2016, 04, 20), new DateTime(2016, 04, 22), roomTypeId, "guest 1", totalNumberOfDoubleRoomsInThisHotel));
            //When
            var checkInDate = new DateTime(2016, 04, 19);
            var checkOutDate = new DateTime(2016, 04, 23);
            object[] events = null;
            TestDelegate when = () => events = rooms.Reserve("guest 2", checkInDate, checkOutDate, totalNumberOfDoubleRoomsInThisHotel).ToArray();
            //Then
            var ex = Assert.Throws<InvalidOperationException>(when);
            Assert.That(events, Is.Null);
            Assert.That(ex.Message, Is.EqualTo("No vacancy."));
        ...
        public void New_reservations_when_there_are_less_existing_reservations_than_there_are_rooms_of_this_type_are_OK()
        {
            ...
            rooms.Hydrate(new RoomsCreated(roomTypeId, "Double Room", totalNumberOfDoubleRoomsInThisHotel, hotelId));
            rooms.Hydrate(new RoomsReserved(new DateTime(2016, 04, 20), new DateTime(2016, 04, 22), roomTypeId, "guest 1", 1));
            rooms.Hydrate(new RoomsReserved(new DateTime(2016, 04, 20), new DateTime(2016, 04, 22), roomTypeId, "guest 3", 1));
            //When
            var checkInDate = new DateTime(2016, 04, 19);
            var checkOutDate = new DateTime(2016, 04, 23);
            var events = rooms.Reserve("guest 2", checkInDate, checkOutDate, totalNumberOfDoubleRoomsInThisHotel).ToArray();
            var roomsReserved = events[0];
            //Then
            Assert.That(roomsReserved, Is.TypeOf<RoomsReserved>());
        }
